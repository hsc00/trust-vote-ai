...
### Validation Steps (run in order before every commit)

| Step       | Command             | When it fails                                               |
| ---------- | ------------------- | ----------------------------------------------------------- |
| Type check | `npm run typecheck` | Fix all TS errors — no `@ts-ignore` shortcuts               |
| Lint       | `npm run lint`      | Fix lint errors; warnings are allowed but review them       |
| Unit tests | `npm run test`      | All tests must pass; add tests for new logic                |
| Build      | `npm run build`     | Catches import errors and missing env vars not caught by TS |
...
- [ ] `npm run test` — all tests green; new logic has tests
- [ ] `npm run build` — build succeeds locally
- [ ] No `any` types introduced
- [ ] No early `return <Spinner />` patterns introduced
- [ ] Lazy-loaded components wrapped in `<SuspenseLoader>`
- [ ] New feature exports added to its `index.ts` barrel
