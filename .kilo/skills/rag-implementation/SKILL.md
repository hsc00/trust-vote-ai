...
- **Qdrant**: Fast, filtered, Rust-based; advanced filtering, open-source
- **pgvector**: PostgreSQL extension, SQL integration; use with existing Postgres

### 2. Embeddings

**Decision Criteria:**

- Choose model based on accuracy, cost, language support, and deployment needs
...
## Concrete Actions for RAG Implementation

To implement a Retrieval-Augmented Generation (RAG) system, perform the following concrete actions:

1. **Chunk Documents**: Split source documents into manageable chunks using strategies such as recursive character, token-based, semantic, or header-based splitting.
2. **Generate Embeddings**: Convert each chunk into a vector representation using a suitable embedding model (e.g., OpenAI, Voyage, BGE, E5).
3. **Index Vectors**: Store the embeddings in a vector database (e.g., Pinecone, Weaviate, Chroma, pgvector) for efficient similarity search.
4. **Perform Similarity Search**: Retrieve relevant chunks by comparing the query embedding to stored vectors using cosine or dot-product similarity.
5. **Implement Hybrid Retrieval**: Combine dense (embedding-based) and sparse (keyword/BM25) retrieval for improved recall and precision.
6. **Apply Metadata Filtering**: Filter results using metadata (e.g., source, category, date) to increase relevance.
7. **Rerank Results**: Use cross-encoder models or reranking APIs to reorder retrieved chunks by relevance.
8. **Assemble Context**: Concatenate top-ranked chunks to form the context for the LLM prompt.
9. **Prompt LLM**: Provide the context and user query to the LLM for grounded answer generation.
10. **Return Citations**: Include source references for transparency and traceability.

These steps are essential for robust, production-grade RAG systems.

## Validation Workflow

Before deploying a RAG system to production, follow this workflow to ensure correctness and quality:

### 1. Verify Embeddings Creation

- Check that all document chunks have corresponding embeddings in the vector store.
- Spot-check a few embeddings for expected dimensionality and non-zero values.
- Optionally, visualize embeddings (e.g., with t-SNE or PCA) to confirm separation by topic.

### 2. Test Retrieval Quality

- Run sample queries against the retriever and inspect the returned chunks for relevance.
- Confirm that top results are semantically related to the query and cover expected ground-truth answers.
- Test both dense and hybrid retrieval (if used) for recall and precision.
- Use metadata filters and verify they restrict results as intended.

### 3. Validate Full Pipeline

- Simulate end-to-end user queries and check that the LLM answers are grounded in retrieved context.
- Ensure citations or source references match the actual retrieved documents.
- Evaluate with a set of test cases (questions + expected answers + relevant doc IDs) and measure retrieval/answer metrics (see Evaluation Metrics section).
- Review edge cases: ambiguous queries, out-of-domain questions, and long/short queries.

### 4. Monitor and Iterate

- Log retrieval and answer quality in staging before production.
